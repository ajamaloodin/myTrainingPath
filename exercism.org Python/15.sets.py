...
def clean_ingredients(dish_name, dish_ingredients):
    ...
    restuple = (dish_name, ingredients)
    return restuple
...
def categorize_dish(dish_name, dish_ingredients):
    ...
    return dish_name + ": undefine Category"
...
def tag_special_ingredients(dish):
    ...
    warning_ingred = ingred.intersection(SPECIAL_INGREDIENTS)
    result = (meal, warning_ingred)
...
def compile_ingredients(dishes):
    ...
    all_ingreds = set()
    for item in dishes:
        all_ingreds = all_ingreds.union(item)
    return all_ingreds
...
def separate_appetizers(dishes, appetizers):
    ...
    setappet = set(appetizers)
    return list(setdishes.difference(setappet))
